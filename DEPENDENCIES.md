...
- **License**: MIT
- **Why needed**:
  - Creates D-Bus service masquerading as a battery BMS
  - Monitors SmartShunt, solar chargers, and MultiPlus devices via DbusMonitor
  - Manages device settings via SettingsDevice
  - Publishes charge control parameters (CVL, CCL, DCL) to control charging/inverting

## Architecture Note

This service converts a Victron SmartShunt into a virtual BMS (Battery Management System) that can control charging and inverting behavior. It monitors battery voltage, temperature, and state of charge, then calculates and publishes appropriate charge voltage limits (CVL), charge current limits (CCL), and discharge current limits (DCL) to protect the battery system.

The service implements sophisticated charge phase control, temperature-based derating, and supports multiple battery chemistries (LFP, LTO, Lead-acid).
